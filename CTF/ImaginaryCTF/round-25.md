...
From the provided `enc.py` file, we can see the flag is encrypted via textbook RSA. The twist is that the program asks us to input our name, and uses what we put in as the random seed. `e`, the public exponent, is generated by doing `2*getrandbits(32)+1`.

> getrandbits(32) will return an integer in the range $[0, 2^{32}-1]$

Since the `getrandbits()` function depends on the random seed, ***which we control***, the objective of this challenge is to get a seed that makes `getrandbits()` generate a small enough number. The reason we want a small number is so that we can use the Chinese Remainder Theorem to get the flag. I consider "small" anything less than 100.

The code below brute forces numbers (which are used as potential seeds) from 0 to 100 million. To speed things up, I used multiprocessing and split the work among 5 processes where each one does 20 million iterations.
...
if __name__ == '__main__':
    processes = []
    numIterations = 20000000 # each process will do 20 million iterations (5 total processes)
    ranges = [range(i, i+numIterations) for i in range(0, 100000000, numIterations)]
    for r in ranges:
...
Since the seed `82660432` yielded the smallest number, we'll use that
